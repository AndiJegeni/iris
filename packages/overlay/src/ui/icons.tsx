...
/**
 * Claude spark (vb 24×24) — Anthropic's official mark, path from the
 * simple-icons set (CC0-licensed path data; the mark itself remains
 * Anthropic's trademark). `currentColor` fill; call sites pass Claude's clay
 * orange where the brand hue is wanted (the accounts sign-in buttons) and
 * inherit ink elsewhere.
 */
export function ClaudeLogoIcon({ size = 16, ...props }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true" {...props}>
      <path
        fill="currentColor"
...
/**
 * OpenAI blossom (vb 24×24) — OpenAI's official mark, path from the
 * simple-icons set (CC0-licensed path data; the mark itself remains OpenAI's
 * trademark). `currentColor` fill: the real mark is monochrome, so it stays
 * ink in both themes.
 */
export function OpenAILogoIcon({ size = 16, ...props }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true" {...props}>
      <path
        fill="currentColor"
